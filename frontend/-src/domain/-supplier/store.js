...
class SupplierStore extends Store {
  ...
  _setup() {
    ...
    this.addReducer('stats/success', (state, payload) => ({ ...state, stats: payload.stats }));
    this.addReducer('top/success', (state, payload) => ({ ...state, topList: payload.list }));
    ...
    this.addEffect('supplier/status', async (payload, _, dispatch) => {
      await this.api.updateStatus(payload.id, payload.status);
      await dispatch({ type: 'list/load' });
    });
    this.addEffect('supplier/rating', async (payload, _, dispatch) => {
      await this.api.updateRating(payload.id, payload.rating);
      await dispatch({ type: 'list/load' });
    });
    ...
    this.addEffect('top/load', async (payload, _, dispatch) => {
      try {
        const list = await this.api.getTop(payload.limit || 10);
        await dispatch({ type: 'top/success', payload: { list } });
      } catch (err) { console.error('加载TOP供应商失败:', err); }
    });
  }

  async load() { await this.dispatch({ type: 'list/load' }); }
  async loadDetail(id) { await this.dispatch({ type: 'detail/load', payload: { id } }); }
  async create(data) { await this.dispatch({ type: 'supplier/create', payload: { data } }); }
  async update(id, data) { await this.dispatch({ type: 'supplier/update', payload: { id, data } }); }
  async updateStatus(id, status) { await this.dispatch({ type: 'supplier/status', payload: { id, status } }); }
  async updateRating(id, rating) { await this.dispatch({ type: 'supplier/rating', payload: { id, rating } }); }
  async delete(id) { await this.dispatch({ type: 'supplier/delete', payload: { id } }); }
  async loadStats() { await this.dispatch({ type: 'stats/load' }); }
  async loadTop(limit = 10) { await this.dispatch({ type: 'top/load', payload: { limit } }); }
